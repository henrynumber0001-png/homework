...
7. 导入后的题目默认未发布，题目列表提供多选操作栏，可批量发布选中题目。
...
Spring Boot 高频面试题            [未发布]          [发布题库] [更多]
面试 / Java / Spring Boot · 100 道题 · 98 道已发布

[题目 100] [题库设置]
──────────────────────────────────────────────────────────────────────────
题目页签：
[搜索题目] [题型] [发布状态]           [调整排序] [Excel 导入] [新建题目]
...
题目当前均为未发布状态，请检查后通过多选操作栏发布。

[查看未发布题目]  [继续导入]
```

点击“查看未发布题目”返回当前题库工作台，并自动带上 `released=false` 筛选。管理员勾选需要发布的题目后，通过多选操作栏执行批量发布。
...
标题最多展示两行，悬停显示完整标题。共享题目显示“被 N 个题库引用”；存在隐藏引用时只提示“还有无权查看的关联题库”，不泄露题库信息。
...
从 Excel 导入完成页返回时，题目列表默认筛选“未发布”，管理员可使用当前页全选，再通过该操作栏批量发布。
...
- 展示管理员有权看到的关联题库。
- `hasHiddenReferences=true` 时展示权限提示。
- 没有全部关联题库权限时，表单进入只读状态。
- 被多个题库引用时不显示删除主体按钮。

### 12.6 发布和删除动作

发布、下架、删除和恢复使用统一动作弹窗。删除题目时明确说明：
...
☰  1  Spring IOC 的核心作用是什么？           [移至序号]
☰  2  Bean 生命周期有哪些阶段？               [移至序号]
☰  3  Spring 事务为什么会失效？               [移至序号]
```
...
- 前端按分页请求加载题库全部未删除题目。
- 使用虚拟列表承载较大题库。
- 支持拖拽排序，也支持输入目标序号快速移动。
- 搜索只用于定位并高亮题目，不从排序数组中移除其他题目。
- 保存时提交完整且不重复的题目 ID 数组。
- 保存期间禁止继续拖动。
- 版本冲突时保留本地顺序，提示“题库顺序已被其他管理员修改”，允许刷新后重新调整。
...
3. 上传非法 Excel → 查看错误 → 替换文件 → 成功导入 → 筛选未发布题目 → 多选并批量发布。
